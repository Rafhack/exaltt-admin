...
// ─── DEFAULT DATA: see src/data/defaults.js ───────────────────────────────────

const _UNUSED_DEFAULT_BRAND = {
...
export default function AdminPanel() {
  ...
  useEffect(() => {
    repository.getConfig().then(setConfig).catch(() => setConfig(buildDefaultConfig()));
  }, []);
  ...
  const persistAndUpdate = useCallback(async (newConfig) => {
    setConfig(newConfig);
    try {
  ...
  const updateSection = useCallback((section) => async (value) => {
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-[#040810]">
        <div className="text-center space-y-3">
          ...
          <p className="text-xs font-black tracking-widest text-slate-500 uppercase">Carregando configuração...</p>
        </div>
      </div>
    );
  }
